        ...
        if fx_item['bank'] == 'BKTWTWTP-004': # 台灣銀行
            ...
            for fx_tr in fx_tr_list:
                ...
                fx_item['currency_dict'][cc] = {
                    'bb_spot':self.format_fx(bb_spot), 'bs_spot':self.format_fx(bs_spot),
            ...
            # currency table
            fx_tr_list = BeautifulSoup(fx_item['currency_dict'], features="lxml").findAll('tr', {'class':'tableContent-light'})
            fx_item['currency_dict'] = {}
            ...
            for fx_tr_bs, fx_tr_bb in zip(fx_tr_list_bs, fx_tr_list_bb):
                ...
                fx_item['currency_dict'][cc] = {
                    'bb_spot':self.format_fx(fx_tr_bb_list[1].get_text()), 'bs_spot':self.format_fx(fx_tr_bs_list[1].get_text()),
            ...
            time_str = "{}-{}-{} {}:{}".format(int(year)+1911, month, day, hour, minute)
            fx_item['last_updated'] = datetime.strptime(time_str, '%Y-%m-%d %H:%M').strftime("%Y-%m-%d %H:%M")
            ...
            for fx_tr in fx_tr_list:
                ...
                fx_item['currency_dict'][cc] = {
                    'bb_spot':self.format_fx(bb_spot.get_text()), 'bs_spot':self.format_fx(bs_spot.get_text()),
            ...
            for fx_tr in fx_tr_list:
                ...
                if len(tt) == 2:
                    bb_spot, bs_spot = self.format_fx(tt[0]), self.format_fx(tt[1])
                else:
                    bb_spot, bs_spot = '-', '-'
                ...
                fx_item['currency_dict'][cc] = {
                    'bb_spot':bb_spot, 'bs_spot':bs_spot, 'bb_cash':bb_cash, 'bs_cash':bs_cash
                }
            ...
            # currency table
            fx_tr_list = BeautifulSoup(fx_item['currency_dict'], features="lxml").findAll('tr')
            fx_item['currency_dict'] = defaultdict(lambda:{'bb_cash':'-', 'bs_cash':'-', 'bb_spot':'-', 'bs_spot':':'})
            ...
            for fx_tr in fx_tr_list:
                ...
                if 'Cash' in cc_tmp:
                    ...
                    fx_item['currency_dict'][cc]['bb_spot'] = self.format_fx(bb)
                    fx_item['currency_dict'][cc]['bs_spot'] = self.format_fx(bs)
            ...
            for fx_tr1, fx_tr2 in zip(fx_tr_list_1, fx_tr_list_2):
                ...
                bb_spot, bb_cash = [ td_bs.find('span').get_text().strip() for td_bs in fx_tr1.findAll('td')[2:4] ]
                cc, _, bs_spot, bs_cash = [ td_bs.find('span').get_text().strip() for td_bs in fx_tr2.findAll('td')[:4] ]
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, bb_spot, bs_spot, bb_cash, bs_cash = [ fx_td.get_text() for fx_td in fx_tr.findAll('td') ]
                cc = cc.split()[0].strip()
            ...
            for fx_tr in fx_tr_list:
                ...
                if 'CASH' == fx_type:
                    ...
                    fx_item['currency_dict'][fx_cc]['bb_spot'] = self.format_fx(fx_bb)
                    fx_item['currency_dict'][fx_cc]['bs_spot'] = self.format_fx(fx_bs)
                else:
                    continue
            ...
            for fx_tr in fx_tr_list:
                ...
                bb_cash, bs_cash, bb_spot, bs_spot = [ fx_td.get_text().strip() for fx_td in fx_tr.findAll('td')[1:] ]
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, bb_spot, bs_spot, bb_cash, bs_cash = [ fx_td.get_text().strip() for fx_td in fx_tr.findAll('td') ]
                cc = cc.split()[-1].strip()
            ...
            time_str = "{}-{}-{} {}:{}".format(year, month, day, hour, minute)
            fx_item['last_updated'] = datetime.strptime(time_str, '%Y-%m-%d %H:%M').strftime("%Y-%m-%d %H:%M")
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, bb_spot, bs_spot, bb_cash, bs_cash = [ fx_td.find('span').find('span').get_text() for fx_td in fx_tr.findAll('div')[:-1] ]
                cc = cc.split()[1].strip()
            ...
            # last updated
            fx_time = BeautifulSoup(fx_item['last_updated'], 'lxml').get_text().strip().split()
            ...
            # last updated
            fx_time = BeautifulSoup(fx_item['last_updated'], features="lxml").get_text().strip().split()
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, bb_spot, bs_spot = [ fx_td.find('span').get_text().strip() for fx_td in fx_tr.findAll('td')[:3] ]
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, bb_cash, bs_cash, bb_spot, bs_spot = [ fx_td.get_text().strip() for fx_td in fx_tr.findAll('td') ]
                cc = cc.split()[1].strip()
            ...
            # last updated
            fx_item['last_updated'] = BeautifulSoup(fx_item['last_updated'], features="lxml").get_text().strip()
            ...
            for fx_tr in fx_tr_list:
                ...
                cc, _, bb_spot, bs_spot, bb_cash, bs_cash, _ = [ fx_td.find('p').get_text().strip() for fx_td in fx_tr.findAll('li') ]
            ...
            for fx_tr in fx_tr_list:
                ...
                fx_item['currency_dict'][cc] = {
                    'bb_spot':self.format_fx(bb_spot.find('div').get_text()), 'bs_spot':self.format_fx(bs_spot.find('div').get_text()),
            ...
            for fx_tr in fx_tr_list:
                ...
                cc_tmp, bb_spot, bb_cash, bs_spot, bs_cash = fx_tr.findAll('div')
